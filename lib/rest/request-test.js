...
export function Request(config) {
  ...
  return (method, action, params) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      // =========== TollfreeVerification Methods ============================
        ...
        resolve({
          ...
          body: {
            ...
            messageSample: '1.Your Plivo verification code is xxxx. 2.Dear <customer name>, you have signed up on Plivo successfully.',
            number: '18339431490',
            optinImageUrl: 'https://www.plivo.com',
            optinType: 'MOBILE_QR_CODE',
            profileUuid: '42f92135-6ec2-4110-8da4-XXXXXXXXXXXX',
            status: 'REJECTED',
            usecase: '2FA',
            usecaseSummary: '1.We send 2FA codes to customers when they signup. 2.Dear <customer name>, you have signed up on Plivo successfully.',
            uuid: 'XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX',
            volume: '100'
          }
        });                                 
      ...
      else if (method == 'POST' && action == 'ComplianceDocument/') {
        resolve({
          response: {},
          body: {
            api_id: 'db342550-7f1d-11e1-8ea7-1231380bc196',
      ...
      // =========== PHONE NUMBER COMPLIANCE REQUIREMENTS ============================
      else if (method == 'GET' && action == 'PhoneNumber/Compliance/Requirements/') {
        resolve({
          response: {},
          body: {
            api_id: 'test-api-id',
            requirement_id: 'req-uuid-123',
            country_iso: params.country_iso || 'IN',
            number_type: params.number_type || 'local',
            user_type: params.user_type || 'business',
            document_types: [
              {
                document_type_id: 'dt-uuid-123',
                name: 'Registration Certificate',
                description: 'Certificate Registration',
                proof_required: true,
                required_fields: [
                  { field_name: 'business_name', friendly_name: 'Legal Name', field_type: 'string', required: true, min_length: 1, max_length: 50 }
                ]
              }
            ]
          }
      ...
      // =========== PHONE NUMBER COMPLIANCE - CREATE ============================
      else if (method == 'POST' && action == 'PhoneNumber/Compliance/') {
        resolve({
          response: {},
          body: {
            api_id: 'test-api-id',
            compliance_id: 'comp-uuid-123',
            message: 'Compliance application created and submitted for review.'
          }
        });
      }

      // =========== PHONE NUMBER COMPLIANCE - LIST ============================
      else if (method == 'GET' && action == 'PhoneNumber/Compliance/') {
        resolve({
          response: {},
          body: {
            api_id: 'pnc-api-list-001',
            meta: {
              limit: params.limit || 20,
              offset: params.offset || 0,
              next: '/v1/Account/MANWVLYTK4ZWU1YTY4ZT/PhoneNumber/Compliance/?limit=20&offset=20',
              previous: null,
              total_count: 2
            },
            compliances: [
              {
                compliance_id: 'pnc-456',
                alias: 'compliance-us-local',
                status: 'approved',
                user_type: 'business',
                country_iso: 'US',
                number_type: 'local',
                ...
                updated_at: '2025-01-15 10:30:00+00:00'
              },
              {
                compliance_id: 'pnc-789',
                alias: 'compliance-de-mobile',
                status: 'draft',
                ...
                callback_method: null,
                rejection_reason: null,
                created_at: '2025-01-16 11:00:00+00:00',
                updated_at: '2025-01-16 11:00:00+00:00'
              }
            ]
          }
        });
      ...
      // =========== PHONE NUMBER COMPLIANCE - GET SINGLE ============================
      // The real API wraps the compliance data inside a "compliance" key.
      else if (method == 'GET' && action == 'PhoneNumber/Compliance/pnc-456/') {
        resolve({
          response: {},
          body: {
            api_id: 'pnc-api-get-001',
            compliance: {
              compliance_id: 'pnc-456',
              alias: 'compliance-us-local',
              status: 'approved',
              ...
              end_user: { end_user_id: 'eu-789', end_user_type: 'business' },
              rejection_reason: null,
              created_at: '2025-01-15 10:30:00+00:00',
              updated_at: '2025-01-15 10:30:00+00:00',
              linked_numbers: []
      ...
      else if (method == 'GET' && action == 'PhoneNumber/Compliance/pnc-rejected/') {
        resolve({
          response: {},
          body: {
            api_id: 'pnc-api-get-002',
            compliance: {
              compliance_id: 'pnc-rejected',
              alias: 'compliance-rejected',
              status: 'rejected',
              user_type: 'business',
              country_iso: 'US',
              number_type: 'local',
              ...
              linked_numbers: ['+14155551234', '+14155555678'],
              created_at: '2025-01-14 09:00:00+00:00',
              updated_at: '2025-01-14 09:00:00+00:00'
            }
          }
        });
      }
      else if (method == 'GET' && action == 'PhoneNumber/Compliance/pnc-not-found/') {
        reject(new Error('not found'));
      }

      // =========== PHONE NUMBER COMPLIANCE - UPDATE ============================
      else if (method == 'PATCH' && action == 'PhoneNumber/Compliance/pnc-456/') {
        resolve({
          response: {},
          body: {
            api_id: 'pnc-api-update-001',
            message: 'Compliance application updated successfully',
            compliance: {
              compliance_id: 'pnc-456',
              alias: params.alias || 'compliance-us-local',
              status: 'draft'
            }
          }
        });
      }
      else if (method == 'PATCH' && action == 'PhoneNumber/Compliance/pnc-submitted/') {
        reject(new Error('Cannot update compliance application in submitted status'));
      }

      // =========== PHONE NUMBER COMPLIANCE - DELETE ============================
      else if (method == 'DELETE' && action == 'PhoneNumber/Compliance/pnc-456/') {
        resolve({
          response: {},
          body: {}
        });
      }
      else if (method == 'DELETE' && action == 'PhoneNumber/Compliance/pnc-not-found/') {
        reject(new Error('not found'));
      }

      // =========== PHONE NUMBER COMPLIANCE - LINK ============================
      else if (method == 'POST' && action == 'PhoneNumber/Compliance/Link/') {
        resolve({
          response: {},
          body: {
            api_id: 'pnc-api-link-001',
            total_count: 1,
            updated_count: 1,
            report: [
              {
                number: params.phone_number || '+14155551234',
                status: 'success',
                remarks: 'Linked successfully'
              }
            ]
          }
        });
