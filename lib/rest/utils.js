...
export function validateStreamAttributes(body, attributes) {

  if (!body) {
    return { success: false, msg: "No body set for Stream" };
  }

  if (attributes.extraHeaders) {
     console.log("Has headers");
     console.log(typeof attributes.extraHeaders);
     console.log(attributes.extraHeaders instanceof Object);
     console.log(Object.keys(attributes.extraHeaders).length);
     console.log(Object.keys(attributes.extraHeaders).length > 0);
     if (attributes.extraHeaders instanceof Object && Object.keys(attributes.extraHeaders).length > 0) {
        var headersDict = {};
        Object.entries(attributes.extraHeaders).forEach(([k,v]) => {
            if (k.endsWith("X-PH")) {
                headersDict[k] = v;
            } else {
                headersDict[k + "X-PH"] = v;
            }
        })
        var str = JSON.stringify(headersDict);
        attributes.extraHeaders = str.replace('&quot;', '/"');
     } else {
        return { success: false, msg: "Invalid data for extraHeaders" };
     }
  }
  return {
    success: true,
