...
export class CallInterface extends PlivoResourceInterface {
    ...
    startStream(serviceUrl, callUuid, params = {}) {
        let errors = validate([{
                field: 'serviceUrl',
                value: serviceUrl,
                validators: ['isRequired']
        ...
        let client = this[clientKey];
        return new Promise((resolve, reject) => {
            client('POST', action + idField + stream, params)
                .then(response => {
                    console.log("Response------>", response);
                    console.log("Response.Body----->", response.body);
                    resolve(new StartStreamResponse(response.body, streamField));
                })
                .catch(error => {
                    reject(error);
                });
     ...
     * @fail {Error} returns Error
     */
    startStream(serviceUrl, callUUID, optionalParams = {}) {
        let errors = validate([{
            field: 'serviceUrl',
            value: serviceUrl,
            validators: ['isRequired']
