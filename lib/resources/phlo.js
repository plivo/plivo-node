...
import { PhloConferenceBridge, PhloConferenceBridgeInterface } from './phloConferenceBridge.js';
...
export class Phlo extends PlivoResource {
  constructor(client, data = {}) {
    ...
    this.conferenceBridge = function (nodeId) {
      let dd = new PhloConferenceBridge(client, { phloId: item.phloId, nodeId: nodeId });
      return dd;
    };

    this.conferenceBridge.get = async function (nodeId) {
      let dd = new PhloConferenceBridgeInterface(client, { phloId: item.phloId, nodeId: nodeId });
      return dd.get(item.phloId, nodeId);
    }
